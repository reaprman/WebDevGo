...
func main() {
	...
	db, err = sql.Open("mysql", "dbuser:password@tcp(mydbinstance.crdqfibloobp.us-east-2.rds.amazonaws.com:3306)/tes02?charset=utf8")
	check(err)
	defer db.Close()
...
func insert(w http.ResponseWriter, req *http.Request) {
	...
	stmt, err := db.Prepare(`INSERT INTO customer VALUES ("John");`)
	check(err)
	defer stmt.Close()
...
func update(w http.ResponseWriter, req *http.Request) {
	...
	stmt, err := db.Prepare(`UPDATE customer SET name="Spartan-117" WHERE name="john";`)
	check(err)
	defer stmt.Close()
...
func del(w http.ResponseWriter, req *http.Request) {
	...
	stmt, err := db.Prepare(`DELETE FROM customer WHERE name="Spartan-117";`)
	check(err)
	defer stmt.Close()
